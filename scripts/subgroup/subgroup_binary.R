...
analyze_treatment_effect_subgroups_binary <- function(data, outcome_var, subgroup_vars, confounders = NULL, outcome_name = "Binary Outcome", dataset_name = NULL) {
  ...
  subgroup_results <- list(); cohort_other_map <- list()
  ...
  for (subgroup_var in subgroup_vars) {
    ...
    res <- tryCatch({
      ...
      if (!is.null(processed$error) && processed$error == "insufficient_levels") {
        ...
        list(interaction_p = model_results$interaction_p, subgroup_effects = subgroup_effects, model = model_results$model,
  ...
  other_map <- if (length(cohort_other_map) > 0) cohort_other_map else {
    var_other_map <- list(); for (var_name in names(subgroup_results)) if (!is.null(subgroup_results[[var_name]]$other_map)) var_other_map[[var_name]] <- subgroup_results[[var_name]]$other_map; var_other_map
  }
