...
get_cutoff_value <- function(var_name, data, percentile_cut = 0.5) {
  if (USE_CLINICAL_BINNING_CONTINUOUS && var_name %in% c("initial_tumor_height", "initial_tumor_diameter")) {
    if (var_name == "initial_tumor_height")       return(T_STAGE_HEIGHT_CUTOFFS)
    if (var_name == "initial_tumor_diameter")     return(T_STAGE_DIAMETER_CUTOFFS)
...
get_subgroup_levels <- function(var_name) {
  if (USE_CLINICAL_BINNING_CONTINUOUS && var_name %in% c("initial_tumor_height", "initial_tumor_diameter")) {
    if (var_name == "initial_tumor_height") {
      return(c(paste0("\u2264 ", T_STAGE_HEIGHT_CUTOFFS[1], " mm"),
      ...
      return(c(paste0("\u2264 ", T_STAGE_DIAMETER_CUTOFFS[1], " mm"),
...
process_subgroup_data <- function(data, subgroup_var, confounders, include_baseline_height = FALSE) {
  ...
  if (was_continuous) {
    ...
    other_map <- list(); subgroup_var_to_use <- subgroup_var
  } else {
...
fit_subgroup_model <- function(data, outcome_config, subgroup_var_to_use, confounders_to_use) {
  interaction_diagnostics <- list(); interaction_p <- NA
  valid_levels <- c(); subgroup_levels <- levels(data[[subgroup_var_to_use]])
  for (level in subgroup_levels) {
    ...
    if (outcome_config$type == "survival") {
      event_vars <- c("death_event", "mets_event", "pfs_event", "event"); event_var <- NULL
      for (ev in event_vars) if (ev %in% names(level_data)) { event_var <- ev; break }
  ...
  if (length(valid_levels) == 0) return(list(model = NULL, interaction_p = NA, formula_used = NA, interaction_diagnostics = interaction_diagnostics, filtered_data = NULL))
  ...
  if (is.null(model)) return(list(model = NULL, interaction_p = NA, formula_used = NA, interaction_diagnostics = list(failure_reason = "Model fitting failed"), filtered_data = filtered_data))
  ...
  if (length(subgroup_levels) == 2) {
    ...
    } else interaction_p <- NA
  } else {
    interaction_test <- tryCatch({ if (outcome_config$type == "survival") anova(no_int, model) else if (outcome_config$type == "binary") anova(no_int, model, test = "Chisq") else anova(no_int, model) }, error = function(e) NULL)
    if (!is.null(interaction_test) && nrow(interaction_test) >= 2) {
      if (outcome_config$type == "survival") interaction_p <- interaction_test[[grep("Pr.*Chi", names(interaction_test), value = TRUE)[1]]][2]
      else if (outcome_config$type == "binary") interaction_p <- interaction_test$`Pr(>Chi)`[2]
      else interaction_p <- interaction_test$`Pr(>F)`[2]
    } else interaction_p <- NA
  }
...
calculate_subgroup_effects <- function(model, data, subgroup_var_to_use, outcome_type, original_var_name) {
  ...
  for (i in seq_along(levels_to_process)) {
    ...
    if (outcome_type == "survival") {
      ...
      event_vars <- c("death_event", "mets_event", "pfs_event", "event"); found_event_var <- NULL
      for (ev in event_vars) if (ev %in% names(level_data)) { found_event_var <- ev; break }
      ...
      plaque_data <- level_data %>% dplyr::filter(treatment_group == "Plaque")
      gksrs_data <- level_data %>% dplyr::filter(treatment_group == "GKSRS")
      outcome_vars <- c("recurrence1", "mets_progression", "outcome"); found_outcome_var <- NULL
      for (ov in outcome_vars) if (ov %in% names(level_data)) { found_outcome_var <- ov; break }
    ...
    if (i == 1) {
      ...
      } else { effect_est <- ci_lower <- ci_upper <- p_val <- NA }
    } else {
      ...
      if (!is.null(main_coef_idx) && !is.null(interaction_coef_idx)) {
        ...
        if (outcome_type == "continuous") {
          effect_est <- combined_coef; ci_lower <- combined_coef - 1.96 * se_combined; ci_upper <- combined_coef + 1.96 * se_combined
        } else {
          effect_est <- exp(combined_coef); ci_lower <- exp(combined_coef - 1.96 * se_combined); ci_upper <- exp(combined_coef + 1.96 * se_combined)
        }
        z_stat <- combined_coef / se_combined; p_val <- 2 * (1 - pnorm(abs(z_stat)))
      } else { effect_est <- ci_lower <- ci_upper <- p_val <- NA }
    }
