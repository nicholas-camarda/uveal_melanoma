...
run_objective_2 <- function(data, dataset_name, output_dirs, prefix, confounders = NULL) {
    ...
    # Retinopathy
    retinopathy_analysis <- analyze_radiation_complications(data, sequela_type = "retinopathy", confounders, dataset_name, output_dirs, prefix)
    logger::log_info(formatted("Retinopathy analysis completed", indent = 1))

    # Neovascular glaucoma
    nvg_analysis <- analyze_radiation_complications(data, sequela_type = "nvg", confounders, dataset_name, output_dirs, prefix)
    logger::log_info(formatted("Neovascular glaucoma analysis completed", indent = 1))

    # Serous retinal detachment
    srd_analysis <- analyze_radiation_complications(data, sequela_type = "srd", confounders, dataset_name, output_dirs, prefix)
