...
# via scripts/utils/all_helper_functions.R and reports cross-file duplicates.

suppressWarnings(suppressMessages({
  library(dplyr)
}))

# Collect sourced files from all_helper_functions.R using base regex
sourced_files <- {
  txt <- readLines('scripts/utils/all_helper_functions.R', warn = FALSE)
  matches <- regmatches(txt, gregexpr('source\("([^"]+)"\)', txt, perl = TRUE))
  found <- unique(unlist(matches))
  if (length(found) == 0) character(0) else {
...
extract_top_level_functions <- function(path) {
  ...
  if (is.null(expr)) return(tibble(file = character(), fun = character()))
  pd <- tryCatch(utils::getParseData(expr), error = function(e) NULL)
  if (is.null(pd)) return(tibble(file = character(), fun = character()))
  ...
  if (length(res) == 0) return(tibble(file = character(), fun = character()))
...
duplicates <- fun_map %>% dplyr::distinct(file, fun) %>% dplyr::count(fun, name = 'file_count') %>% dplyr::filter(file_count > 1)
...
if (nrow(duplicates) == 0) {
  ...
  fun_map %>% dplyr::semi_join(duplicates, by = 'fun') %>% dplyr::arrange(fun, file) %>%
    dplyr::group_by(fun) %>% dplyr::summarise(files = paste(unique(file), collapse = ', ')) %>%
    dplyr::mutate(msg = paste0(' - ', fun, ': ', files)) %>% dplyr::pull(msg) %>% cat(sep = '\n')
  cat('\n')
}
