...
The standalone no-GEP exploratory report lives under `04_GEP_Validation/d_exploratory_no_gep/` and is generated by `run_exploratory_no_gep_report("uveal_melanoma_full_cohort")`. It is intentionally separate from the main Objective 4 validation pipeline because it addresses a different question: what clinically useful risk information can still be produced when no definitive GEP result is available?

This workflow keeps `GEP Failed/Indeterminate` and `GEP Not Tested` separate in the main summaries. It first restores the same display labels and GEP status flags used elsewhere in the project, verifies the simplified no-GEP KM risk-table fix against the expected cohort counts, and then builds three modeling populations:
...
The derived binary `ciliary_involvement` field is also retained for interpretation and prediction-sheet context. It is derived from `location` using case-insensitive matching of `cilio|ciliary`, which correctly captures source values such as `Cilio-Choroidal`.

Before fitting the exploratory models, predictors are screened using shared project utilities:

- factor levels are first collapsed with the existing rare-category handling
- retained predictors must have at least 90% non-missingness in the surrogate, direct-MFS, and direct-MSS modeling datasets
- categorical predictors are dropped if, after collapse, any non-reference level still has fewer than 5 observations in one of those datasets
- rows with retained factor values collapsed to `Other` are removed with the shared `exclude_other_categories()` helper before model fitting

The retained-versus-dropped predictor list is written into the workbook and summary text so the modeling denominator is transparent.

#### Exploratory model definitions

Three ridge-penalized logistic regression models are fit with `glmnet::cv.glmnet()` and `alpha = 0`:
...
All three models use the same retained baseline predictor set so interpretation stays aligned across outputs. Ridge penalty selection is internal to each model via cross-validation. The workbook reports:
...
The coefficient tables in this report are ridge-shrunken terms at `lambda.min`. They are included for directionality and relative contribution only; they are not inferential p-value summaries.
...
#### Predictor contribution summaries

The exploratory workbook adds a predictor-contribution ranking derived from the standardized absolute ridge coefficients. For each retained source predictor, the report identifies the strongest design-matrix term and ranks predictors by the magnitude of that shrunken coefficient. This is intended to answer which baseline features are most influential in the exploratory risk ordering, not which predictors are “statistically significant.”
