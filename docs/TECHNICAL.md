...
| **1f. Tumor Height (Sensitivity)** | Same model with baseline tumor height added | `analyze_tumor_height_changes()` | Regression models (.html) and diagnostics (.xlsx) | `{cohort}/01_Efficacy/f_tumor_height_sensitivity/` |
...
Tumor-height interpretation note: baseline-height sensitivity addresses baseline tumor-size imbalance only. It does not correct unequal time from treatment to follow-up height assessment; the timing audit in `e_tumor_height_primary/` is required context before using tumor-height comparisons in reviewer-facing text.
