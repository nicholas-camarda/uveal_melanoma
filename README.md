...
Objective 4 validates externally supplied lab-reported GEP survival probabilities for MFS and MSS. The pipeline copies the lab-reported 5-year survival values into the 5-year `expected_*` columns, extrapolates 7- and 10-year survival during preprocessing, and converts survival to event risk for observed-vs-expected, calibration, discrimination, and decision-curve analyses. It does not fit a new prognostic model to generate the base GEP predictions. MSS uses melanoma-specific death as the primary event definition, with competing non-melanoma death handled in the companion competing-risk analyses.
