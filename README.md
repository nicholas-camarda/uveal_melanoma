...
This software provides a comprehensive platform for analyzing clinical outcomes in uveal melanoma patients treated with either Gamma Knife Stereotactic Radiosurgery (GKSRS) or Proton Beam Therapy (PBT) brachytherapy.
...
**Definition:** Patients eligible for **both** treatment modalities**Eligibility Criteria:**
...
**Definition:** Patients **ineligible** for PBT brachytherapy**Exclusion Criteria:**
...
**Method:** Interaction testing across patient subgroups for treatment effect heterogeneity**Implementation:** Unified `subgroup_analysis.R` with dedicated functions:
...
**Subgroups:** Age, sex, tumor location, initial tumor height/diameter**Forest Plots:** Comprehensive forest plot visualization with `create_single_cohort_forest_plot()`**Outputs:**
...
**Status:** Planned (implementation in progress)**Method (planned):** Multi-timepoint validation (5, 7, 10 years) with comprehensive statistical testing:
...
**Status:** Planned (implementation in progress)**Method (planned):** Standard survival analysis plus Fine-Gray competing risk models with cumulative incidence functions. The analysis will employ a **robust discrimination strategy** that focuses on metrics that work reliably with our data distribution:
