test_that("event/date reconciliation writes a single row-level review workbook into 00_General", {
    ...
    expect_false("Manual_Date_Corrections" %in% readxl::excel_sheets(audit_paths$xlsx_path))
...
test_that("loader exposes no record-specific raw-source override mechanism", {
    expect_false(exists("MANUAL_DATE_CORRECTIONS", inherits = TRUE))
    expect_false(exists("apply_manual_date_corrections", mode = "function", inherits = TRUE))
})
