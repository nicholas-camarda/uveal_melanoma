...
test_that("figure counts audit generator renders current-state runtime summary language", {
    ...
    expect_match(doc_text, "`runtime/Analytic Dataset/cohort_summary_statistics.json`", fixed = TRUE)
    expect_match(doc_text, "`runtime/Analysis/uveal_full/00_General/removed_patients_summary.tsv`", fixed = TRUE)
    expect_no_match(doc_text, "~/Workspaces", fixed = TRUE)
    expect_no_match(doc_text, "OneDrive-Personal", fixed = TRUE)
